...
export async function uploadFile(fileId, blob) {
  const resp = await fetch(`${API_BASE}/file/${encodeURIComponent(fileId)}`, {
    method: "POST",
    headers: {
  ...
  if (!resp.ok) {
    const text = await resp.text().catch(() => "");
    throw new Error(
      `File upload failed: HTTP ${resp.status} ${resp.statusText} ${text.slice(0, 200)}`,
    );
  }
  return resp.json();
}
...
export async function getFileBlob(fileId) {
  ...
  if (!resp.ok) {
    throw new Error(
      `File fetch failed: HTTP ${resp.status} ${resp.statusText}`,
    );
  }
  return resp.blob();
}
...
export async function deleteFile(fileId) {
  ...
  if (!resp.ok && resp.status !== 404) {
    throw new Error(
      `File delete failed: HTTP ${resp.status} ${resp.statusText}`,
    );
  }
  return true;
}
