...
export function openBookModal(bookId) {
  ...
  renderBookModalFileRow();
  openModal("bookModal");
}

// The file row is only meaningful for a book that already exists -- a new one
// has no fileId to attach bytes to until it is saved.
export function renderBookModalFileRow() {
  const group = document.getElementById("bookModalFileGroup");
  if (!group) return;

  const book = bookModalState.editingBookId
    ? getBookById(bookModalState.editingBookId)
    : null;
  group.style.display = book ? "" : "none";
  if (!book) return;

  const hasFile = !!booksBlobStatus[book.bookId];
  const nameEl = document.getElementById("bookModalFileName");
  if (nameEl) {
    nameEl.textContent = hasFile
      ? `${book.fileName || "book.pdf"} · ${formatByteSize(book.fileSize || 0)}`
      : "No PDF on this device";
    nameEl.classList.toggle("is-missing", !hasFile);
  }

  const button = document.getElementById("bookModalFileBtn");
  if (button) {
    button.textContent = hasFile ? "Replace file…" : "Choose PDF file…";
  }

  const hint = document.getElementById("bookModalFileHint");
  if (hint) {
    hint.textContent = hasFile
      ? "Picking another file keeps every bookmark, note and summary — only the pages change."
      : "Pick this book's PDF from your device storage. Bookmarks stay exactly as they are.";
  }
}

export function chooseFileForEditedBook() {
  if (!bookModalState.editingBookId) return;
  chooseBookFile(bookModalState.editingBookId);
}
